...
def objective(trial):
    ...
    scheduler2 = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[4,9,14], gamma=gamma_up)
...
#             start = time.time()
            if task.name == "EEG-IMG-DIFFUSION":
                args_dict = {
                    "model" : model,
...
#                 latent_dict = results["latent_dict"]
                ...
                # Early Stopping
                if results['dev_loss'] > st_loss * 1.1 or results['dev_loss'] > st_loss + 0.05 or best_epoch + 5 < epoch:
                    return results['dev_loss']
        scheduler1.step()
        scheduler2.step()
    
        if results['dev_loss'] < best_loss:
            torch.save(model.state_dict(), "./tune_checkpoints/BEST-BART+DIFFUSION.pt")
            best_loss = results['dev_loss']
        
    return results['dev_loss']

study = optuna.load_study(study_name="DIFFUSION-15", storage="sqlite:///OPTUNA-DB/DIFFUSION-15.db")
