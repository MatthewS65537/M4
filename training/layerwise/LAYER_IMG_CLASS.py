...
#             print()
    ...
    train_writer = SummaryWriter(log_dir=f"{log_dir}/train-layerwise-class-only-D0.5-FINAL")
    dev_writer = SummaryWriter(log_dir=f"{log_dir}/dev-layerwise-class-only-D0.5-FINAL")
...
#         )
    ...
    while epoch < num_epochs:
        ...
        for task in dsg_tasks.tasks:
            print(f">>>> {task.name}")
            if task.name == "EEG-IMG-CLASSIFICATION":
                args_dict = {
                    "model" : model,
                    "dataloader" : dataset_dict[task.dataset_tag],
                    "optimizer" : task.optimizer(model.parameters(), betas=(0.99, 0.999), eps=6.7e-7, weight_decay=6.3e-7, lr=task.learning_rate),
